// Last updated: 18/08/2026, 03:19:44
1class Solution {
...
8
9         for(int num : nums){  //O(N)
10            mini = Math.min(mini ,num);
11            maxi= Math.max(maxi,num);
12            set.add(num);
13         }
14
15         for(int i = mini ;i <= maxi; i++){ // O(K)
...
20    }
21}
22//Time complexity : O(N * K)// The second loop runs K times (from mini to maxi).
